...
namespace AS_Store_GL.Controllers
{
    public class SaleController : AppController
    {
        ...
        public ActionResult EditOrder(PageModel model, string command)
        {
            ...
            else if (command == "Save")
            {
                ...
                if (findTransNO.Count != 0)
                {
                    ...
                    model.StkTransmst.COMPID = model.StkTrans.COMPID;
                    model.StkTransmst.TRANSDT = model.StkTrans.TRANSDT;
                    model.StkTransmst.TRANSYY = model.StkTransmst.TRANSYY;
                    model.StkTransmst.TRANSNO = Convert.ToInt64(model.StkTrans.TRANSNO);
                    model.StkTransmst.INSUSERID = Convert.ToInt64(System.Web.HttpContext.Current.Session["loggedUserID"]);
                    SendingCustomSMS(model);
        ...
        // Send custom SMS
        public void SendingCustomSMS(PageModel model)
        {
            ...
            if (userName != null && usPss != null && senderName != null)
            {
                try
                {
                    ...
                    var find_TRANSNO_CustomSMS = (from m in db.CustomSmsDbSet
                                                  where m.COMPID == model.StkTransmst.COMPID && m.TRANSDT == model.StkTransmst.TRANSDT
                                                  && m.TRANSYY == model.StkTransmst.TRANSYY && m.TRANSNO == model.StkTransmst.TRANSNO && m.STATUS == "PENDING" && m.SMSTP == "SALE"
                                                  select m).ToList();
                    if (find_TRANSNO_CustomSMS.Count == 0)
                    {
                        var findPSID = (from m in db.PsDbSet
                                        where m.COMPID == model.StkTransmst.COMPID && m.PSID == model.AGL_acchart.ACCOUNTCD && m.MOBNO != null
        ...
        public void Insert_ASL_CSMS_Table(PageModel currentSmsBody, string mobileNo)
        {
            ...
            smsLogData.TRANSNO = Convert.ToInt64(currentSmsBody.StkTransmst.TRANSNO);
            smsLogData.SMSTP = "SALE";
            if (mobileNo != null)
            {
                smsLogData.MOBNO = mobileNo;
            }
            smsLogData.LANGUAGE = "ENG";
            smsLogData.MESSAGE = "Dear Customer, You have purchased Total TK. " + currentSmsBody.StkTransmst.TOTNET + ", cash received TK. " + currentSmsBody.StkTransmst.CASHAMT + " and Your Due balance of Invoice " + currentSmsBody.StkTransmst.TRANSNO + " is TK. " + currentSmsBody.StkTransmst.CREDITAMT + ", with Regards," + companyName + ".";
        ...
        public void Update_ASL_CSMS_Table_SendingSms(ASL_CSMS model, PageModel currentSmsBody)
        {
            var updateTable =
